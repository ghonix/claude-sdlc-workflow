...
```
Agent(subagent_type="sdlc-researcher", description="Research: [task summary]", prompt="[full task description from the user]")
```

After the researcher completes, read `.sdlc/1-research.md` and present a **brief summary** to the user:
- Key files involved
- Risks identified
- Open questions (if any)

**Ask the user**: "Research is complete. Review `.sdlc/1-research.md` for full details. Should I proceed to planning, or do you want to adjust the research?"

### Phase 2: Plan

Invoke the planner agent:
```
Agent(subagent_type="sdlc-planner", description="Plan: [task summary]", prompt="Read .sdlc/1-research.md and create an implementation plan. Task: [task description]")
```

After the planner completes, read `.sdlc/2-plan.md` and present:
...
**Ask the user**: "Plan is ready. Review `.sdlc/2-plan.md` for the full plan. Should I proceed to QA, or do you want to revise the plan?"

### Phase 3: QA (Shift-Left)

Invoke the QA agent:
```
Agent(subagent_type="sdlc-qa", description="QA: [task summary]", prompt="Read .sdlc/1-research.md and .sdlc/2-plan.md. Define acceptance criteria and test plan. Task: [task description]")
```

After QA completes, read `.sdlc/3-qa.md` and present:
- Number of acceptance criteria
- Critical edge cases identified
- Any gaps found in the plan

**Ask the user**: "QA brief is ready. Review `.sdlc/3-qa.md` for acceptance criteria and test plan. Should I proceed to implementation?"

### Phase 4: Implement (Parallel Waves)

Read the **Execution Graph** from `.sdlc/2-plan.md` to determine the wave structure.

#### Single-wave plan (all steps sequential or only 1 wave)

Invoke one implementer for the full plan:
```
Agent(subagent_type="sdlc-implementer", description="Implement: [task summary]", prompt="Read .sdlc/1-research.md, .sdlc/2-plan.md, and .sdlc/3-qa.md. Implement all steps. Task: [task description]")
```
...
Agent(subagent_type="sdlc-implementer", description="Implement S1", prompt="Implement step S1 only. Read .sdlc/1-research.md, .sdlc/2-plan.md, .sdlc/3-qa.md. Task: [task description]", run_in_background=true)
Agent(subagent_type="sdlc-implementer", description="Implement S2", prompt="Implement step S2 only. Read .sdlc/1-research.md, .sdlc/2-plan.md, .sdlc/3-qa.md. Task: [task description]", run_in_background=true)

# Wait for Wave 1 to complete, then launch Wave 2
Agent(subagent_type="sdlc-implementer", description="Implement S3", prompt="Implement step S3 only. Read .sdlc/1-research.md, .sdlc/2-plan.md, .sdlc/3-qa.md. Task: [task description]")
```
...
After all waves complete, read all `4-implementation-*.md` files and present:
...
```
Agent(subagent_type="sdlc-verifier", description="Verify: [task summary]", prompt="Read all .sdlc/*.md artifacts and verify the implementation. Run tests and code review. Task: [task description]")
```

After verification completes, read `.sdlc/5-verification.md` and present the final verdict:
...
3. **Pass the task description forward** — every agent invocation should include the original task description.
4. **Create .sdlc directory** — run `mkdir -p .sdlc` before the first phase if it doesn't exist.
